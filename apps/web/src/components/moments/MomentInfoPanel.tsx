...
import { Heart, Clock3, MapPin, MessageCircle, Share2, Bookmark, Check, Eye, Send } from "lucide-react";
...
export function MomentInfoPanel({
  ...
  const remaining = formatRemaining(moment.expiresAt ?? null, moment.isPermanent ?? false);
  ...
  return (
    <div className="flex min-w-0 flex-col space-y-4 rounded-3xl border border-slate-200 bg-white p-5 md:p-6">
      ...
      <div className="space-y-1 border-y border-slate-100 py-3 text-sm text-slate-600">
        ...
        {remaining ? <p className="flex items-center gap-2"><Clock3 className="h-4 w-4 text-slate-400" />{remaining}</p> : null}
        <p className="flex items-center gap-2 text-slate-400">{formatRelative(moment.createdAt)}</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-slate-600">
        <span className="inline-flex items-center gap-1"><Heart className={`h-4 w-4 ${liked ? "fill-rose-500 text-rose-500" : "text-slate-400"}`} />{moment.stats.likes} Me gusta</span>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          <Heart className={`h-4 w-4 ${liked ? "fill-rose-500" : ""}`} />{liked ? "Te gusta" : "Me gusta"}
        </button>
        <button
          onClick={onConfirm}
        ...
        >
          <Check className="h-4 w-4" />{confirmed ? "Confirmado" : "Confirmar"}
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
        ...
        <button onClick={onSave} aria-pressed={saved} className={`inline-flex items-center justify-center gap-1.5 rounded-xl border px-3 py-2.5 font-medium transition ${saved ? "border-indigo-300 bg-indigo-50 text-indigo-700" : "border-slate-300 bg-white text-slate-700 hover:bg-slate-50"}`}>
          <Bookmark className={`h-4 w-4 ${saved ? "fill-indigo-500" : ""}`} />{saved ? "Guardado" : "Guardar"}
        </button>
