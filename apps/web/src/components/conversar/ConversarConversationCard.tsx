...
export function ConversarConversationCard({
  ...
  const isFinished = conversation.status === "finished" || conversation.status === "recorded";

  return (
    <article
      className={`space-y-4 rounded-3xl border bg-white p-5 shadow-soft transition hover:-translate-y-0.5 hover:shadow-md ${
        isLive
    ...
    >
      ...
      <div>
        ...
        <p className="mt-1 text-sm leading-relaxed text-slate-600">{conversation.description}</p>
      </div>

      <div className="grid gap-2 rounded-2xl border border-slate-200 bg-slate-50/70 p-3 text-sm text-slate-700 sm:grid-cols-2">
        <p>
          Creado por <span className="font-semibold text-slate-900">{conversation.createdBy.name}</span>
        </p>
      ...
      <div className="flex items-center justify-between gap-3 border-t border-slate-100 pt-2">
        ...
        <div className="flex flex-wrap gap-2">
          <PrimaryButton
            ...
            onClick={() => onPrimaryAction?.(conversation)}
          >
