...
export function ConversarDebateStanceColumn({ stance }: Props) {
  const members = stance.arguments
    .slice(0, 3)
    .map((argument) => argument.author.name);

  return (
    <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
      <h3 className="text-lg font-bold text-slate-900">{stance.title}</h3>
      {stance.description ? (
        <p className="mt-1 text-sm text-slate-600">{stance.description}</p>
      ) : null}
      <p className="mt-3 text-sm font-semibold text-slate-700">
        {stance.participants} participantes
      </p>
