...
export function ConversarFinishedHeader({
  conversation,
}: {
  ...
  const metadata = [
    ...
    conversation.recording?.durationLabel
      ? `Duración: ${conversation.recording.durationLabel}`
      : null,
  ]
    .filter(Boolean)
    .join(" · ");
  return (
    <header className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
      ...
      <p className="mt-3 inline-block rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-emerald-800">
      ...
      <p className="mt-1 text-sm font-medium text-slate-700">
        {getRecordingStatusLabel(conversation.recording?.status)}
      </p>
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        >
          Crear conversación relacionada
        </Link>
        <button
          type="button"
          className="rounded-xl bg-slate-200 px-4 py-2 text-sm font-semibold text-slate-700"
