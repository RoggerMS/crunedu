...
export function ConversarCompanionCard({ companion, invitePrepared, onInvite }: Props) {
  return (
    <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-indigo-100 text-sm font-black text-indigo-700">{getInitials(companion.user.name)}</div>
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        <PrimaryButton type="button" onClick={onInvite}>{invitePrepared ? "Invitación preparada" : "Invitar a conversar"}</PrimaryButton>
        <SecondaryButton type="button" disabled>Ver perfil</SecondaryButton>
      </div>
    </article>
  );
}
