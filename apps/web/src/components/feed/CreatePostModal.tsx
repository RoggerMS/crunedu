...
export function CreatePostModal(props: { open: boolean; initialType: PostType; communities: CommunityLite[]; isAuthenticated: boolean; onClose: () => void; onSaveDraft: (data: CreatePostSubmitPayload) => void; onSubmit: (data: CreatePostSubmitPayload) => void; onToast: (message: string, type: "success" | "error" | "info") => void }) {
  ...
  return <div className="fixed inset-0 z-50 bg-slate-950/45 p-3 backdrop-blur-sm" onClick={props.onClose}><div role="dialog" aria-modal className="mx-auto mt-3 max-h-[93vh] w-full max-w-[1000px] overflow-y-auto rounded-3xl bg-white p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
    ...
    <div className="mt-4 grid gap-4 lg:grid-cols-3"><div className="space-y-3 lg:col-span-2">
      ...
      <div className="flex flex-wrap gap-2"><button type="button" title="Los documentos se publican desde Apuntes." onClick={() => props.onToast("Para subir PDF, Word o PPT usa la sección Apuntes. Luego podrás compartirlo en el feed.", "info")} className="cursor-not-allowed rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-500">Adjuntar archivo</button>
      <label className="cursor-pointer rounded-xl border px-3 py-2 text-xs font-semibold">Adjuntar imagen<input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (!f) return; if (images.length >= MAX_FEED_IMAGES) return props.onToast(`Por ahora puedes subir hasta ${MAX_FEED_IMAGES} imágenes.`, "info"); const mediaId = crypto.randomUUID(); setImages((prev) => [...prev, { id: mediaId, mediaId, previewUrl: URL.createObjectURL(f) }]); }} /></label></div>
      <FeedAttachmentPreview files={[]} images={images} onRemoveImage={(id) => setImages((prev) => prev.filter((img) => img.id !== id))} />
      {!hasContent ? <p className="text-xs text-rose-600">Agrega texto o una imagen para publicar.</p> : null}
