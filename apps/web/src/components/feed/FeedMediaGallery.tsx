...
export function FeedMediaGallery({ images, onSelect }: { images: MediaItem[]; onSelect?: (index: number) => void }) {
  ...
  const renderItem = (image: MediaItem, className: string, w: number, h: number, index: number) => !image.previewUrl ? <button key={image.id} onClick={() => onSelect?.(index)} className="flex h-full min-h-24 w-full cursor-pointer items-center justify-center rounded-xl border border-dashed text-xs text-slate-500">{image.unavailableLabel ?? "Imagen pendiente de sincronización."}</button> : <button key={image.id} onClick={() => onSelect?.(index)} className="group relative w-full cursor-pointer overflow-hidden rounded-xl"><img src={image.previewUrl} alt={image.alt ?? "Imagen"} width={w} height={h} className={`${className} transition group-hover:brightness-95`} loading="lazy" />{image.type === "video" ? <span className="absolute bottom-2 right-2 rounded bg-black/70 px-2 py-1 text-[10px] text-white">Video</span> : null}</button>;
