...
import { ImagePlus, Loader2, MessageSquarePlus, NotebookPen, Sparkles, X } from "lucide-react";
...
const publishOptions = [
  { id: "publicacion", label: "Publicación", helper: "Comparte ideas o novedades." },
  { id: "apunte", label: "Apunte", helper: "Comparte apuntes y materiales.", href: "/app/apuntes/nuevo" },
  { id: "pregunta", label: "Pregunta", helper: "Haz una pregunta a tu comunidad.", href: "/app/preguntas/nuevo" },
  { id: "momento", label: "Momento memorable", helper: "Comparte experiencias.", href: "/app/momentos" },
  { id: "debate", label: "Debate", helper: "Inicia un debate sobre un tema.", href: "/app/debates/crear" },
  { id: "tramite", label: "Trámite", helper: "Comparte trámites y convocatorias.", href: "/app/tramites" },
] as const;
...
export default function AppPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFormError(err instanceof Error ? mapApiError(err, "No se pudo publicar. Verifica tus datos.") : "Ocurrió un error inesperado.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return <div className="relative space-y-5">
    <Card className="space-y-4">
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
        <button type="button" onClick={() => setIsCreateFormOpen(true)} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-left text-sm font-semibold text-slate-500 hover:border-indigo-300">¿Qué quieres compartir con tu comunidad?</button>
      </div>
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Link href="/app/apuntes/nuevo" className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"><NotebookPen size={16} /> Apunte</Link>
        <Link href="/app/preguntas/nuevo" className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">Pregunta</Link>
        <Link href="/app/debates/crear" className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"><Sparkles size={16} /> Debate</Link>
      </div>
      <div className="flex flex-wrap items-center gap-2 border-t border-slate-200 pt-3 text-xs font-semibold text-slate-600">
    ...
    {isCreateFormOpen ? <div className="fixed inset-0 z-40 flex items-start justify-center bg-slate-900/35 p-4 pt-14">
      <div className="w-full max-w-5xl rounded-3xl border border-slate-200 bg-white p-5 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-black">¿Qué quieres compartir?</h2>
          <button type="button" onClick={() => setIsCreateFormOpen(false)} className="rounded-lg p-1 text-slate-500 hover:bg-slate-100"><X size={18} /></button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-4 grid gap-4 lg:grid-cols-3">
          <div className="space-y-4 lg:col-span-2">
            <FormField><p className="mb-1 text-sm font-semibold text-slate-700">Escribe tu publicación</p><TextArea value={content} onChange={(event) => setContent(event.target.value)} rows={7} placeholder="Comparte algo útil con tu comunidad..." required /></FormField>
            <FormField><p className="mb-1 text-sm font-semibold text-slate-700">Agregar imágenes</p><Input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => void handleAttachImage(event.target.files?.[0] ?? null)} disabled={!isAuthenticated || uploadingImage || attachedImages.length >= 4} /><div className="mt-2 grid grid-cols-4 gap-2">{attachedImages.map((image) => <img key={image.storageKey} src={image.imageUrl} alt="Vista previa" className="h-16 w-full rounded-xl object-cover" />)}</div></FormField>
          </div>
          <div className="space-y-4">
            <FormField><p className="mb-1 text-sm font-semibold text-slate-700">Comunidad</p><Select value={communityId} onChange={(event) => setCommunityId(event.target.value)} required><option value="">Selecciona una comunidad</option>{communities.map((community: Community) => <option key={community.id} value={community.id}>{community.name}</option>)}</Select></FormField>
            <Card className="space-y-2 border-dashed p-3"><p className="text-xs font-bold text-slate-700">Etiquetas</p><div className="flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-indigo-100 px-2 py-1 text-indigo-700">estudios</span><span className="rounded-full bg-indigo-100 px-2 py-1 text-indigo-700">resumen</span></div></Card>
            ...
            <div className="flex gap-2"><PrimaryButton type="submit" disabled={!canSubmit || submitting} className="w-full">{submitting ? <Loader2 className="animate-spin" size={16} /> : <ImagePlus size={16} />} Publicar</PrimaryButton></div>
          </div>
        </form>
      </div>
    </div> : null}
    ...
    <div className="mt-6 grid gap-4 xl:grid-cols-12">{loading ? <StatusMessage type="loading">Cargando publicaciones...</StatusMessage> : null}{error ? <StatusMessage type="error">{error}</StatusMessage> : null}
      ...
      <aside className="hidden space-y-4 xl:col-span-4 xl:block">
        <Card className="space-y-3"><h3 className="text-sm font-black text-slate-800">Comunidades recomendadas</h3>{communities.slice(0, 4).map((community) => <div key={community.id} className="flex items-center justify-between rounded-xl border border-slate-200 px-3 py-2"><p className="text-sm font-semibold text-slate-700">{community.name}</p><Link href={`/app/comunidades/${community.id}`} className="inline-flex items-center justify-center rounded-2xl border border-slate-300 px-3 py-1 text-xs font-semibold text-slate-700 transition hover:bg-slate-50">Unirse</Link></div>)}</Card>
        <Card className="space-y-3"><h3 className="text-sm font-black text-slate-800">Temas en tendencia</h3><p className="rounded-xl bg-slate-50 p-3 text-xs text-slate-600">#ParcialEstadística · #SesiónDeTiempo · #Cachimbos</p></Card>
      </aside>
    </div>
  </div>;
}
