...
export default function AppPage() {
  ...
  return <div className="relative mx-auto w-full max-w-[1500px] space-y-4 px-2 lg:px-4">
    ...
    <FeedPostModal open={Boolean(activePost)} post={activePost} comments={activePost ? (feed.commentsByPost[activePost.id] ?? []) : []} onClose={() => setActivePostId(null)} onComment={(postId, content, parentId) => void feed.addComment(postId, content, parentId)} onLikePost={(postId) => { const liked = activePost?.viewerState.liked; void feed.likePost(postId); showToast(liked ? "Me gusta quitado." : "Me gusta registrado.", "info"); }} onSavePost={(postId) => { const saved = activePost?.viewerState.saved; void feed.savePost(postId); showToast(saved ? "Guardado quitado." : "Publicación guardada.", "info"); }} onSharePost={async (postId) => { const shareUrl = `${window.location.origin}/app?post=${postId}`; await navigator.clipboard.writeText(shareUrl); await feed.sharePost(postId); showToast("Enlace copiado.", "success"); }} onReportPost={handleReport} onHidePost={(postId) => { void feed.hidePost(postId); setActivePostId(null); }} onDeletePost={(postId) => { void feed.deletePost(postId); setActivePostId(null); }} onLikeComment={(postId, commentId) => void feed.likeComment(postId, commentId)} />
  </div>;
}
