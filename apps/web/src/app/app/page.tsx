...
const POSTS_KEY = "crunedu_feed_posts";
const LIKES_KEY = "crunedu_feed_likes";
const SAVES_KEY = "crunedu_feed_saves";
const COMMENTS_KEY = "crunedu_feed_comments";
const REPORTS_KEY = "crunedu_feed_reports";
const HIDDEN_KEY = "crunedu_feed_hidden";
...
export default function AppPage() {
  ...
  useEffect(() => {
    ...
    setLocalPosts(JSON.parse(localStorage.getItem(POSTS_KEY) ?? "[]"));
  }, []);
  useEffect(() => { localStorage.setItem(POSTS_KEY, JSON.stringify(localPosts)); }, [localPosts]);
  ...
  return <div className="relative space-y-4">
    ...
    <CreatePostModal open={isOpen} initialType={selectedType} communities={communities} isAuthenticated={isAuthenticated} onClose={() => setIsOpen(false)} onToast={showToast}
      onSaveDraft={(data) => { const draft: PostDraft = { id: crypto.randomUUID(), type: data.type, title: data.title, content: data.content, courseName: data.courseName, stance: data.stance, deadline: data.deadline, visibility: data.visibility, communityId: data.communityId || undefined, tags: data.tags, attachedFiles: data.attachedFiles.map((f) => ({ ...f, file: undefined })), attachedImages: data.attachedImages, createdAt: new Date().toISOString() }; const current = JSON.parse(localStorage.getItem(DRAFTS_KEY) ?? "[]") as PostDraft[]; localStorage.setItem(DRAFTS_KEY, JSON.stringify([draft, ...current])); setDraftsCount(current.length + 1); showToast("Borrador guardado.", "success"); }}
      onSubmit={async (data) => {
        ...
        setLocalPosts((prev) => [optimistic, ...prev]); setIsOpen(false); showToast("Publicación creada.", "success");
        if (!accessToken) return showToast("Inicia sesión para publicar contenido.", "error");
        try {
          const uploadedImages: CreatePostImagePayload[] = []; const payload: CreateFeedPostPayload = { content: data.content.trim() };
          if (data.communityId) payload.communityId = Number(data.communityId);
          else payload.communityId = 1;
        ...
        } catch { showToast("Publicación guardada localmente. Se sincronizará cuando el backend esté listo.", "info"); }
      }} />
    ...
    <div className="grid gap-4 xl:grid-cols-12">
      <section className="space-y-3 xl:col-span-8">
        {filteredPosts.map((post) => <PostCard key={post.id} post={post as FeedPost | LocalFeedPost} onLike={(id) => updateLocalPost(id, (current) => { const liked = !current.viewerState.liked; const likes = Math.max(0, current.stats.likes + (liked ? 1 : -1)); localStorage.setItem(LIKES_KEY, JSON.stringify({ [id]: likes })); return { ...current, viewerState: { ...current.viewerState, liked }, stats: { ...current.stats, likes } }; })} onSave={(id) => updateLocalPost(id, (current) => { const saved = !current.viewerState.saved; const saves = Math.max(0, current.stats.saves + (saved ? 1 : -1)); localStorage.setItem(SAVES_KEY, JSON.stringify({ [id]: saves })); showToast(saved ? "Publicación guardada" : "Quitada de guardados", "success"); return { ...current, viewerState: { ...current.viewerState, saved }, stats: { ...current.stats, saves } }; })} onComment={(id, comment) => updateLocalPost(id, (current) => { const next = { ...current, commentsPreview: [{ id: crypto.randomUUID(), authorName: "Tú", content: comment, createdAt: new Date().toISOString() }, ...(current.commentsPreview ?? [])], stats: { ...current.stats, comments: current.stats.comments + 1 } }; localStorage.setItem(COMMENTS_KEY, JSON.stringify({ [id]: next.commentsPreview })); return next; })} onShare={async (id) => { try { await navigator.clipboard.writeText(`${window.location.origin}/app/posts/${id}`); localStorage.setItem("crunedu_events", JSON.stringify([{ type: "share", id, date: new Date().toISOString() }])); showToast("Enlace copiado", "success"); } catch { showToast("No se pudo copiar el enlace.", "error"); } }} />)}
