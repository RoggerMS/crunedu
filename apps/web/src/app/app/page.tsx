...
export default function AppPage() {
  ...
  return <div className="relative space-y-4">
    ...
    <CreatePostModal open={isOpen} initialType={selectedType} communities={communities} isAuthenticated={isAuthenticated} onClose={() => setIsOpen(false)} onToast={showToast}
      ...
      onSubmit={async (data) => {
        if (!data.content.trim() && data.attachedImages.length === 0) return showToast("Agrega texto, imagen o video para publicar.", "error");
        ...
        const attachments: FeedAttachment[] = data.attachedImages.map((img) => ({ id: img.id, type: "image", name: "media", mimeType: "image/*", size: 0, previewUrl: img.previewUrl, storageKey: img.mediaId }));
    ...
    <div className="grid gap-4 xl:grid-cols-12"><section className="space-y-3 xl:col-span-8">
      {filteredPosts.map((post) => <PostCard key={post.id} post={post}
        ...
        onShare={async (id) => { try { await navigator.clipboard.writeText(`http://localhost:3000/app?post=${id}`); await feed.sharePost(String(id)); showToast("Enlace copiado.", "success"); } catch { showToast("No se pudo copiar el enlace.", "error"); } }}
    ...
    <FeedPostModal open={Boolean(activePost)} post={activePost} comments={activePost ? (feed.commentsByPost[activePost.id] ?? []) : []} onClose={() => setActivePostId(null)} onComment={(postId, content) => void feed.addComment(postId, content)} />
  </div>;
}
