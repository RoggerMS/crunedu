    ...
    <div className="mx-auto grid max-w-[1600px] gap-4 px-6 py-4 xl:grid-cols-[minmax(0,1fr)_320px]">
      <main className="space-y-4">
        ...
        <StoreSection title="Para tu vida académica" actionLabel="Ver más">
          <StoreListingGrid items={getSectionListings(academicListings, 6)} onSave={store.saveListing} onContact={store.contactSeller} onShare={store.shareListing} onReport={store.reportListing} onHide={store.hideListing} />
        </StoreSection>

        <StoreSection title="Servicios rápidos en campus" actionLabel="Ver más">
          <StoreListingGrid items={getSectionListings(servicesListings, 4)} onSave={store.saveListing} onContact={store.contactSeller} onShare={store.shareListing} onReport={store.reportListing} onHide={store.hideListing} />
        </StoreSection>

        <StoreSection title="Emprendimientos estudiantiles" actionLabel="Ver más">
          <StoreListingGrid items={getSectionListings(businessListings, 4)} onSave={store.saveListing} onContact={store.contactSeller} onShare={store.shareListing} onReport={store.reportListing} onHide={store.hideListing} />
        </StoreSection>

        <StoreSection title="Intercambios y donaciones" actionLabel="Ver más">
          <StoreListingGrid items={getSectionListings(socialListings, 4)} onSave={store.saveListing} onContact={store.contactSeller} onShare={store.shareListing} onReport={store.reportListing} onHide={store.hideListing} />
        </StoreSection>
        ...
        <StoreSection title="Explorar todo" subtitle="Filtra por categoría, entrega y ordenamiento.">
          ...
          {!store.loading && store.filteredListings.length > 0 ? <StoreListingGrid items={store.filteredListings} onSave={store.saveListing} onContact={store.contactSeller} onShare={store.shareListing} onReport={store.reportListing} onHide={store.hideListing} /> : null}
        </StoreSection>
      </main>
