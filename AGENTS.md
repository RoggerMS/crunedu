...
| `docs/MOMENTOS_PLAN.md` | Momentos MVP (compose, image, boost toggle, future integration) | In progress | Alta: 2026-05-03. Actualizado 2026-06-26: módulo completo persistente — schema (Moment, MomentMedia, MomentTag, MomentBoost, MomentConfirmation, SavedMoment, MomentComment), migración `20260626120000_moments_module`, backend NestJS completo (list/detail/create/update/delete, media upload/serve, boost/confirm/save/share, comments, news, gallery, saved, trends, topics con JWT/RateLimit/cursor/_count), seed idempotente con 6 momentos, frontend real (moments-api.ts, useMoments con loading/error/reintento/optimistic+rollback, 5 vistas conectadas, MomentForm compartido, detalle con API). Pruebas `moments.smoke.ts`. Pendiente: aplicar migración+seed y QA runtime local con Docker. |
