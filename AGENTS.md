...
| `docs/QUESTIONS_FORUM_PLAN.md` | Preguntas: backend robusto + integración con Inicio, Comunidades y Apuntes | In progress | Alta: 2026-05-03. Actualizado 2026-06-19: backend completo (filtros communityId/q/status incluido `answered`, PATCH/DELETE, rate limiting, isMine, viewerVote, canMarkUseful); editor WYSIWYG (`RichAcademicEditor` + `SymbolPicker` + `EquationPicker`) reemplaza Markdown/vista previa; rediseño listado 3 columnas, cards, ranking colaboradores; detalle con galería, votos up/down, marcar correcta gated por `canMarkUseful`; validación plain-text. Fase 2 (feed integration) y Fase 3 (apuntes/adjuntos) pendientes. Puntos/guardar/asignaturas dedicadas diferidos (requieren schema). |
| `docs/MOMENTOS_PLAN.md` | Momentos MVP (compose, image, boost toggle, future integration) | In progress | Alta: 2026-05-03. Actualizado 2026-06-27: arquitectura canónica — `Post` es publicación canónica, `Moment` es placement que referencia `postId`; Me gusta (`Reaction`) reemplaza Impulsar (migración backfill `20260627000000_canonical_post_moment_link`); confirmar solo en Momentos; expiración nullable + `isPermanent`; noticias con detalle (`/api/moments/news/:id`, ruta `/app/momentos/noticias/[id]`); preferencia de visualización; navegación por teclado; fallback controlado por `NEXT_PUBLIC_MOMENTS_USE_FALLBACK`. Endpoints like/save/share en posts y moments. Pruebas `moments.smoke.ts`. Pendiente: aplicar migraciones+seed y QA runtime local con Docker. |

| `docs/STORE_CORE_PLAN.md` | Tienda núcleo (MVP gestionado por CrunEdu + base para marketplace futuro) | In progress | Alta: 2026-05-04. Actualizado 2026-06-19: Fase 1 completada — schema extendido (ProductType/PriceType/Condition/DeliveryType enums, nuevos campos, ProductSafePoint, imágenes robustas), backend completo (catalog con filtros/search/sort+viewerState, CRUD público con JWT, publish/pause/mark-sold, favorites toggle/list, product reports, inquiries sin datos manuales, image upload/serve, me endpoints con estadísticas, admin reports+métricas), frontend rediseñado (URL-synced filters, cards con imágenes reales/fallback Lucide, detail con galería+lightbox, publish funcional con upload, save/favorite/contact/report reales, panel personal drawer, admin gating), seed con 10 categorías, 4 safe points, 12 productos. Migración `20260619190000_store_functional_v1` pendiente de aplicar localmente. Fases 2-4 (conversaciones, reservas, transacciones, reseñas) pendientes. |
...
| `docs/APUNTES_MODULE_PLAN.md` | Apuntes funcional: backend real + frontend real (upload, descarga, guardar, valorar, reportar, visibilidad, comunidad + feed) + mejoras visuales/accesibilidad + previsualizaciones | In progress | Alta: 2026-06-19. Actualizado 2026-06-19: backend hardening (rate limiting, upload verification, visibility checks, search, error handling, body limits, post on visibility update, range requests), frontend visual polish (header gradient, card hover/clickable, mobile filter drawer, /nuevo form page, 2-col detail, EditNoteModal, keyboard nav, a11y), previsualizaciones (NotePreview con lazy loading via Intersection Observer, PDF object/iframe, imagen img+lightbox, tarjeta decorativa para otros tipos, range requests backend). Pendiente: crear/aplicar migración Prisma localmente y verificación runtime con Docker. |
| `docs/UNIVERSITY_MODULE_PLAN.md` | Universidad MVP+: schema extendido, backend completo (overview, calendar, search, saves, reminders, ICS, admin), frontend hub con calendario real conectado a BD, filtros URL, paneles de datos reales | In progress | Alta: 2026-06-20. Fases 1-4 completadas (schema+seed, backend endpoints, frontend hub, calendario completo). Fase 5 backend/calendario completada. Fase 6 pendiente. Actualizado 2026-06-20: auditoría final aplicada; calendario con vistas mes/semana/agenda, agenda diaria real, detalle modal, guardados/recordatorios/ICS funcionales, filtros seguros, solapamiento de rangos corregido y endpoints admin JWT para crear/editar items de calendario. |
| `docs/WORKSPACE_COPY_SOCIAL_INTEGRATION_PLAN.md` | Integración de feed, configuración, perfiles públicos y notificaciones desde `workspace-copiar` | In progress | Alta: 2026-06-21. Implementación completada y builds validados; pendiente QA runtime local. Excluye Preguntas, Momentos, Universidad, Debates y Tienda. |
| `docs/CONVERSAR_IMPLEMENTATION_STATUS.md` | Conversar: salas de audio en tiempo real (LiveKit + backend + frontend + debates + compañeros + grabaciones) | In progress | Alta: 2026-06-30. Fases 1-8 implementadas: schema + migración, backend NestJS completo (40+ endpoints), LiveKit v1.8.0 en Docker, frontend conectado sin mocks, seed, 28 pruebas passed. Pendiente: activar Egress para grabación real end-to-end y prueba manual con dos usuarios. |
| `docs/PROFILE_IMPLEMENTATION_STATUS.md` | Sistema de perfiles: schema extendido, MinIO storage, avatar/portada, perfil unificado con pestañas, visibilidad de posts, seguidores/amigos batch, UserIdentityLink | In progress | Alta: 2026-06-30. Fases 1-4 implementadas: migración aplicada, StorageService MinIO, 40+ endpoints, FeedAuthor con avatarUrl, UserIdentityLink, PublicProfile con tabs, avatar menu en header. |
| `docs/ADMIN_IMPLEMENTATION_STATUS.md` | Sistema de administración integral: seguridad, backend admin, sesiones, auditoría, dashboard y módulos admin | In progress | Alta: 2026-07-01. Fase 1-2 funcional: AdminModule registrado, guard/permisos/step-up, modelos AdminSession/AdminAuditLog/Promotion/ContentPlacement, dashboard real y shell admin; pendiente QA Docker local y UI CRUD avanzada por módulo. |
| `docs/ADMIN_RUNBOOK.md` | Operación del panel administrativo y procedimientos seguros | In progress | Alta: 2026-07-01. Runbook inicial creado; pendiente actualizar con evidencias runtime locales tras Docker. |
| `docs/ADMIN_SECURITY_CHECKLIST.md` | Checklist de seguridad del panel administrativo | In progress | Alta: 2026-07-01. Controles principales documentados; pendiente cierre QA runtime ADMIN/USER y cookie HttpOnly futura. |
